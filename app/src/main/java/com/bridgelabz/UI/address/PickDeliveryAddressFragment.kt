...
class PickDeliveryAddressFragment() : Fragment() {
    ...
    var afterAddressSubmitted: (() -> Unit)? = null
    ...
    private fun pickAddress(it: View) {
        ...
        if (!isMobileValid(mobileNumber)) {
            ...
            Toast.makeText(it.context, "Pin code is not valid", Toast.LENGTH_LONG).show()
            return
        } else {
            ...
            if (args!!.getBoolean("addAddressChecked")) {
                afterAddressSubmitted?.invoke()
                args.putBoolean("addAddressChecked", false)
            } else {
